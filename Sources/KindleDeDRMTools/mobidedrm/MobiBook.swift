...
final class MobiBook {
    ...
    private var numSections: UInt16
    private var sections: BookSections = .init()
    private var metaArray: MetaDictionary = .init()
    private var sect: Data = .init()
    private var records: UInt16 = .zero
    private var compression: UInt16 = .zero
    var mobiData: Data = .init()
    private var cryptoType: Int = -1
    private var printReplica: Bool = false
    private var extraDataFlags: UInt16 = .zero
    private var mobiLength: UInt32 = .zero
    private var mobiCodepage: UInt32 = 1252
    ...
    init(data: Data) throws {
        ...
        header = dataFile.subdata(in: 0..<78)

        Debug.print("header:", header.formattedForOutput)

        magic = header.subdata(in: 0x3C..<0x3C + 8)
        ...
        numSections = header.withUnsafeBytes { $0.load(fromByteOffset: 76, as: UInt16.self).bigEndian }

        Debug.print("numSections:", numSections.description)

        for i in 0..<Int(numSections) {
            let base: Data.Index = 78 + i * 8
            let slice: Data = dataFile.subdata(in: base..<base + 8)

            let offset: UInt32 = slice.withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }
            let a1: UInt8 = slice.withUnsafeBytes { $0.load(fromByteOffset: 4, as: UInt8.self) }
            let a2: UInt8 = slice.withUnsafeBytes { $0.load(fromByteOffset: 5, as: UInt8.self) }
            let a3: UInt8 = slice.withUnsafeBytes { $0.load(fromByteOffset: 6, as: UInt8.self) }
            let a4: UInt8 = slice.withUnsafeBytes { $0.load(fromByteOffset: 7, as: UInt8.self) }

            let flags: UInt8 = a1
            let val: UInt32 = (.init(a2) << 16) | (.init(a3) << 8) | .init(a4)

            let section: BookSection = .init(offset: offset,
                                             flags: flags,
                                             val: val)
        ...
        records = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x8, as: UInt16.self).bigEndian }
        compression = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x0, as: UInt16.self).bigEndian }
        ...
        mobiLength = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x14, as: UInt32.self).bigEndian }
        mobiCodepage = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x1C, as: UInt32.self).bigEndian }
        mobiVersion = .init(sect.withUnsafeBytes { $0.load(fromByteOffset: 0x68, as: UInt32.self).bigEndian })
        ...
        if mobiLength >= 0xE4 && mobiVersion >= 5 {
            extraDataFlags = sect.withUnsafeBytes { $0.load(fromByteOffset: 0xF2, as: UInt16.self).bigEndian }
        }
        ...
        let exthFlag: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x80, as: UInt32.self).bigEndian }
        ...
        if (exthFlag & 0x40) != 0 {
            let range: Range<Data.Index> = 16 + .init(mobiLength)..<sect.count
            exth = sect.subdata(in: range)
        }

        Debug.print("exth:", exth.formattedForOutput)

        if exth.count >= 12 && exth.subdata(in: 0..<4) == CharMaps.exthBytes {
            let nItems: UInt32 = exth.withUnsafeBytes { $0.load(fromByteOffset: 8, as: UInt32.self).bigEndian }
            var pos: Data.Index = 12

            Debug.print("nItems:", nItems.description)

            for _ in 0..<nItems {
                let typeRange: Range<Data.Index> = pos..<pos + 4
                let sizeRange: Range<Data.Index> = pos + 4..<pos + 8

                let type: UInt32 = exth.subdata(in: typeRange).withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }
                let size: UInt32 = exth.subdata(in: sizeRange).withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }

                Debug.print("type:", type.description)
                Debug.print("size:", size.description)

                let contentRange: Range<Data.Index> = pos + 8..<pos + .init(size)
                ...
                case 208:
                    newContent = .init(count: .init(size) - 8)
                default:
                    newContent = nil
                }

                if let newContent {
                    patchSection(section: 0,
                                 newContent: newContent,
                                 inOff: 16 + .init(mobiLength) + pos + 8)
                }
    ...
    private func loadSection(section: Int) -> Data {
        let endoff: Data.Index = section + 1 == .init(numSections) ? dataFile.count : .init(sections[section + 1].offset)
        let off: Data.Index = .init(sections[section].offset)
        let range: Range<Data.Index> = off..<endoff

        return dataFile.subdata(in: range)
    }

    private static func getSizeOfTrailingDataEntries(ptr: Data, size: Int, flags: UInt16) -> Int {
        var num: Int = 0
        var testflags: UInt16 = flags >> 1
    ...
    private func patchSection(section: Int, newContent: Data, inOff: Int = 0) {
        let endoff: Int = (section + 1 == .init(numSections)) ? dataFile.count : .init(sections[section + 1].offset)
        let off: Int = .init(sections[section].offset)
    ...
    private static func parseDrmRoutine(data: Data,
        ...
        for i in 0..<count {
            let startIndex: Data.Index = i * 0x30
            let range: Range<Data.Index> = startIndex..<startIndex + 0x30

            let buffer: Data = data.subdata(in: range)

            let verification: UInt32 = buffer.withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }
            let size: UInt32 = buffer.withUnsafeBytes { $0.load(fromByteOffset: 4, as: UInt32.self).bigEndian }
            let type: UInt32 = buffer.withUnsafeBytes { $0.load(fromByteOffset: 8, as: UInt32.self).bigEndian }
            ...
            if cksum == tempKeySum {
                ...
                let ver: UInt32 = cookie.withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }
                let flags: UInt32 = cookie.withUnsafeBytes { $0.load(fromByteOffset: 4, as: UInt32.self).bigEndian }

                let finalKey: Data = cookie.subdata(in: 8..<8 + 16)

                let expiry1: UInt32 = cookie.withUnsafeBytes { $0.load(fromByteOffset: 24, as: UInt32.self).bigEndian }
                let expiry2: UInt32 = cookie.withUnsafeBytes { $0.load(fromByteOffset: 28, as: UInt32.self).bigEndian }
...
extension MobiBook: BookManager {
    func getBookTitle() -> String {
        let codecMap: [UInt32: String.Encoding] = [1252: .windowsCP1252, 65001: .utf8]
        ...
        if magic == CharMaps.bookmobiBytes {
            if let data: Data = metaArray[503] {
                ...
                let toff: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x54, as: UInt32.self).bigEndian }
                let tlen: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0x58, as: UInt32.self).bigEndian }
                let tend: UInt32 = toff + tlen
                let range: Range<Data.Index> = .init(toff)..<Int(tend)

                title = sect.subdata(in: range)
            }
    ...
    func processBook(pidSet: OrderedSet<String>) throws {
        cryptoType = .init(sect.withUnsafeBytes { $0.load(fromByteOffset: 0xC, as: UInt16.self).bigEndian })
        ...
        if cryptoType == 0 {
            ...
            let range: Range<Data.Index> = 0..<4

            printReplica = data.subdata(in: range) == CharMaps.mopBytes
        ...
        if let data406: Data = metaArray[406] {
            let val406: UInt64 = data406.withUnsafeBytes { $0.load(as: UInt64.self).bigEndian }
        ...
        if cryptoType == 1 {
            ...
            if magic == CharMaps.textreadBytes {
                ...
                bookKeyData = sect.subdata(in: .init(mobiLength) + 16..<Int(mobiLength) + 32)
            }

            pid = "00000000"
            foundKey = try PukallCipher.pc1(key: t1Keyvec, src: bookKeyData)
        } else {
            let drmPtr: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0xA8, as: UInt32.self).bigEndian }
            let drmCount: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0xAC, as: UInt32.self).bigEndian }
            let drmSize: UInt32 = sect.withUnsafeBytes { $0.load(fromByteOffset: 0xB0, as: UInt32.self).bigEndian }

            if drmCount == 0 {
                throw MobiBookError.encryptionNotInitialised
            }

            let drmData: Data = sect.subdata(in: .init(drmPtr)..<Int(drmPtr) + .init(drmSize))

            let drmResult: DRMInfo = try Self.parseDrm(data: drmData, count: .init(drmCount), pidSet: goodPids)
            ...
            patchSection(section: 0, newContent: .init(count: .init(drmSize)), inOff: .init(drmPtr))
        ...
        let rangeToWrite: Data = dataFile.subdata(in: 0..<Int(sections[1].offset))
        decryptedData.append(rangeToWrite)

        for i in 1...Int(records) {
        ...
        if numSections > records + 1 {
            let rangeToWrite: Data = dataFile.subdata(in: .init(sections[.init(records) + 1].offset)..<dataFile.count)
            decryptedData.append(rangeToWrite)
        }
    ...
    func getPidMetaInfo() -> PIDMetaInfo {
        ...
        if let data: Data = metaArray[209] {
            ...
            for i in stride(from: 0, to: rec209.count, by: 5) {
                let startIndex: Data.Index = i + 1
                let range: Range<Data.Index> = startIndex..<startIndex + 4
                let val: UInt32 = rec209.subdata(in: range).withUnsafeBytes { $0.load(as: UInt32.self).bigEndian }
